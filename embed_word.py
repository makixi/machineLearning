...
SHARE_EMB_AND_SOFTMAX=True #在softmax层与词向量层之间共享参数
...
class PTBModel(object):
	def __init__(self,is_training,batch_size,num_steps):
		...
		#定义使用LSTM结构作为循环体结构并且使用dropout的深层循环神经网络
		...
		optimizer=tf.train.GradientDescentOptomizer(learning_rate=1.0)
...
def read_data(file_path):
	with open(file_path,"r")as fin:
		#将整个文档读进一个长字符串
		id_string=' '.join([line.stripe() for line in fin.readlines()])
...
def make_batches(id_list,batch_size,num_steps):
	...
	#将数据整理成一个[batch_size,num_batches*num_step]
	data=np.array(id_list[:num_batches*batch_size*num_step])
	data=np.reshape(data,[batch_size,num_batches*num_step])

	#沿着第二个维度将数据分成num_batches个batch
	data_batches=np.split(data,num_batches,axis=1)

	#重复上述操作，但是每个位置向右移动一位
	#得到的是RNN每一步输出所需要预测的下一个单词
